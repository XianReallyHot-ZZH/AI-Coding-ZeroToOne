...
本文档基于 `0001_spec_by_GLM5.md` 需求设计文档，提供详细的实现步骤和开发指南。项目代码将放置在 `./week01/project-alpha` 目录下。
...
class TicketNotFoundException(HTTPException):
    def __init__(self, ticket_id: int):
        super().__init__(
            ...
            detail={"code": "NOT_FOUND", "message": f"Ticket with id {ticket_id} not found"}
        )
...
class LabelNotFoundException(HTTPException):
    def __init__(self, label_id: int):
        super().__init__(
            status_code=status.HTTP_404_NOT_FOUND,
            detail={"code": "NOT_FOUND", "message": f"Label with id {label_id} not found"}
        )
...
class DuplicateLabelNameException(HTTPException):
    def __init__(self, name: str):
        super().__init__(
            ...
            detail={"code": "CONFLICT", "message": f"Label with name '{name}' already exists"}
        )
```
...
3. **StatusFilter.tsx** - 状态筛选下拉
   - 状态选项
   - 选择回调

4. **LabelFilter.tsx** - 标签筛选侧边栏
   - 标签列表
   - 多选功能
   - 显示 Ticket 数量

5. **TicketCard.tsx** - Ticket 卡片
   - 显示标题、描述预览
   - 显示标签
   - 操作按钮（完成/取消完成、编辑、删除）

6. **TicketList.tsx** - Ticket 列表容器
   - 渲染 TicketCard 列表
   - 加载状态
   - 空状态

7. **Pagination.tsx** - 分页组件
   - 页码显示
   - 翻页控制

8. **TicketForm.tsx** - Ticket 表单
   - 标题输入
   - 描述输入
   - 标签选择
   - 提交验证

9. **LabelManager.tsx** - 标签管理弹窗
   - 标签列表
   - 创建/编辑/删除标签

10. **EmptyState.tsx** - 空状态提示
...
export function TicketCard({ ticket, onComplete, onEdit, onDelete }: TicketCardProps) {
  return (
...
2. **TicketCreatePage.tsx** - 创建页面
   - TicketForm 组件
   - 创建提交逻辑

3. **TicketEditPage.tsx** - 编辑页面
   - 加载现有 Ticket 数据
   - TicketForm 组件
   - 更新提交逻辑
    ...
    <div className="flex h-screen">
      ...
      <main className="flex-1 p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <Button onClick={() => navigate('/tickets/new')}>新建 Ticket</Button>
        </div>
